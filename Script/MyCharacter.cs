...
public class MyCharacter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (coliders[i].isTrigger)
            {
                //Debug.Log("hot trigger");
                SoundTrigger trigger = coliders[i].gameObject.GetComponent<SoundTrigger>();
                if (trigger)
                {
                    //Debug.Log("call func trigger");
                    trigger.TriggerSound(); 
                }
            }
            else if (coliders[i] != _Collider)
            {
